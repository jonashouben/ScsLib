...
namespace ScsLib.Hashing
{
	public class CityHash : ICityHash
	{
		...
		private static uint Fetch32(ReadOnlySpan<byte> data)
		{
			return BitConverter.ToUInt32(data);
		}

		private static ulong Fetch64(ReadOnlySpan<byte> data)
		{
		...
		private static ulong HashLen0To16(ReadOnlySpan<byte> s, int len)
		{
			...
			{
				ulong a = Fetch32(s);
				return Hash128To64(((ulong)len + (a << 3), Fetch32(s.Slice(len - 4))));
			}
		...
		private static ulong HashLen17To32(ReadOnlySpan<byte> s, int len)
		{
			...
			ulong c = Fetch64(s.Slice(len - 8)) * K2;
			ulong d = Fetch64(s.Slice(len - 16)) * K0;
		...
		{
			ulong z = Fetch64(s.Slice(24));
			ulong a = Fetch64(s) + (((ulong)len + Fetch64(s.Slice(len - 16))) * K0);
			ulong b = Rotate(a + z, 52);
			ulong c = Rotate(a, 37);
			a += Fetch64(s.Slice(8));
			c += Rotate(a, 7);
			a += Fetch64(s.Slice(16));
			ulong vf = a + z;
			ulong vs = b + Rotate(a, 31) + c;
			a = Fetch64(s.Slice(16)) + Fetch64(s.Slice(len - 32));
			z = Fetch64(s.Slice(len - 8));
			b = Rotate(a + z, 52);
			c = Rotate(a, 37);
			a += Fetch64(s.Slice(len - 24));
			c += Rotate(a, 7);
			a += Fetch64(s.Slice(len - 16));
			ulong wf = a + z;
		...
		private static (ulong, ulong) WeakHashLen32WithSeeds(ReadOnlySpan<byte> s, int len, ulong a, ulong b)
		{
			return WeakHashLen32WithSeeds(Fetch64(s.Slice(len)),
										  Fetch64(s.Slice(len + 8)),
										  Fetch64(s.Slice(len + 16)),
										  Fetch64(s.Slice(len + 24)),
										  a,
										  b);
		}

		private static ulong CityHash64(ReadOnlySpan<byte> s, int len)
		{
			if (len <= 16) return HashLen0To16(s, len);
			if (len <= 32) return HashLen17To32(s, len);
			if (len <= 64) return HashLen33To64(s, len);

			ulong x = Fetch64(s.Slice(len - 40));
			ulong y = Fetch64(s.Slice(len - 16)) + Fetch64(s.Slice(len - 56));
			ulong z = Hash128To64((Fetch64(s.Slice(len - 48)) + (ulong)len, Fetch64(s.Slice(len - 24))));
			...
			do
			{
				x = Rotate(x + y + v.Item1 + Fetch64(s.Slice(pos + 8)), 37) * K1;
				y = Rotate(y + v.Item2 + Fetch64(s.Slice(pos + 48)), 42) * K1;
				x ^= w.Item2;
				y += v.Item1 + Fetch64(s.Slice(pos + 40));
				z = Rotate(z + w.Item1, 33) * K1;
				v = WeakHashLen32WithSeeds(s, pos, v.Item2 * K1, x + w.Item1);
				w = WeakHashLen32WithSeeds(s, pos + 32, z + w.Item2, y + Fetch64(s.Slice(pos + 16)));
				Swap(ref z, ref x);
